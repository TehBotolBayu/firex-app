...
async function findEmergencyDetail(req, res){
    const result = await client.db("baru").collection("collection_emergencies").aggregate(
        [
            {
                $lookup: {
                  from: "collection_sensors",
                  localField: "sensorId",
                  foreignField: "_id",
                  as: "sensorData"
                }
              ...
              {
                $unwind: "$sensorData" // If needed, to flatten the result
              },
              {
                $lookup: {
                  from: "collection_user",
                  localField: "sensorData.userId",
                  foreignField: "_id",
                  as: "userData"
                }
            }
...
module.exports = {createEmergency, findEmergency, findEmergencies, deleteEmergency, updateEmergency, findEmergencyDetail};